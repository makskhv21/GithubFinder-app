...
export default function Search(props: HeaderProps & UserNicknameProps & UserInfoProps) {
  const [statusCode , setStatusCode] = useState(200);

  const request = async () => {
    try{ const response = await axios.get(
        `https://api.github.com/users/${props.userNickname}`
      );
...
const SearchContainer = styled.div<{ isDarkTheme: boolean }>`
  width: 330px;
  height: 60px;
  display: flex;
  align-items: center;
